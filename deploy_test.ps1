...
gcloud run deploy $SERVICE_NAME `
    ...
    --timeout 3600s `
    --port 8080 `
    --set-env-vars "^|^SUPABASE_URL=https://tlzilbpgwfeushniddkb.supabase.co|SUPABASE_SERVICE_ROLE_KEY=$SUPABASE_SERVICE_ROLE_KEY|GCS_SERVICE_ACCOUNT_JSON=$GCS_JSON|LARK_APP_ID=$LARK_APP_ID|LARK_APP_SECRET=$LARK_APP_SECRET|NHANH_APP_ID=$NHANH_APP_ID|NHANH_BUSINESS_ID=$NHANH_BUSINESS_ID|NHANH_ACCESS_TOKEN=$NHANH_ACCESS_TOKEN|NHANH_PRODUCT_APP_ID=$NHANH_PRODUCT_APP_ID|NHANH_PRODUCT_BUSINESS_ID=$NHANH_PRODUCT_BUSINESS_ID|NHANH_PRODUCT_ACCESS_TOKEN=$NHANH_PRODUCT_ACCESS_TOKEN"
