...
export function WarehouseTable({
  ...
  const addMutation = useMutation({
    mutationFn: async ({ code, name }: { code: string; name: string }) => {
      await postJson<{ ok: true }>("/api/admin-resources/warehouses", {
        code,
        name,
      });
    },
  ...
  const editMutation = useMutation({
    mutationFn: async ({ id, name }: { id: string; name: string }) => {
      await postJson<{ ok: true }>(
        `/api/admin-resources/warehouses/${id}`,
        { name },
        { method: "PUT" },
      );
    },
  ...
  const toggleMutation = useMutation({
    mutationFn: async (id: string) => {
      await postJson<{ ok: true; active: boolean }>(
        `/api/admin-resources/warehouses/${id}/toggle-active`,
        {},
        { method: "PATCH" },
      );
    },
                  ...
                  <div className="flex justify-center gap-1">
                    {editing?.id === w.id ? (
                      <>
                        <LinkBtn
                          onClick={() =>
                            editMutation.mutate({ id: w.id, name: editName })
                          }
                        >
                      ...
                      <>
                        ...
                        {canManage && (
                          <ActionIconButton
                            action={w.active ? "disable" : "enable"}
                            label={`${w.active ? "Vô hiệu hóa" : "Kích hoạt"} kho ${w.code}`}
                        ...
                        {canDelete && (
                          <ActionIconButton
                            action="delete"
                            label={`Xóa kho ${w.code}`}
                            onClick={() => {
                              if (
                                window.confirm(
                                  `Xóa kho "${w.name}"? Không thể hoàn tác.`,
                                )
                              )
              ...
              {canManage &&
                (editing?.id === warehouse.id ? (
                  ...
                  <div className="mt-3 flex gap-2">
                    ...
                    />
                    <ActionIconButton
                      action={warehouse.active ? "disable" : "enable"}
                      label={`${warehouse.active ? "Vô hiệu hóa" : "Kích hoạt"} kho ${warehouse.code}`}
                      onClick={() => toggleMutation.mutate(warehouse.id)}
                    />
                    {canDelete && (
                      <ActionIconButton
                        action="delete"
                        label={`Xóa kho ${warehouse.code}`}
                        onClick={() => {
                          if (
                            window.confirm(
                              `Xóa kho "${warehouse.name}"? Không thể hoàn tác.`,
                            )
                          )
