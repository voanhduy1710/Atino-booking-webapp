...
export default function ReceiverPage() {
  ...
  const lookupBooking = async (tokenStr: string) => {
    ...
    setIsLooking(true)
    try {
      // Extract token from QR format: ATINO:{code}:{token}
      ...
      if (error || !data) {
        setLookupError('Không tìm thấy booking')
        return
      }
      ...
      for (const item of b.items) {
        init[item.id] = item.quantity_booked
      }
      setQuantities(init)
    } finally {
      setIsLooking(false)
            ...
            <div className="px-6 py-4 border-b border-[#E0E0E0] flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-[#888888]">
                  {booking.supplier_name} • {formatDateDisplay(booking.delivery_date)}
                </p>
              </div>
