...
function itemPhotos(item: any, type: "vat_invoice" | "delivery_slip") {
  ...
  if (type === "vat_invoice" && photos.length === 0 && item.vat_invoice_url)
    return [item.vat_invoice_url];
  return photos;
}
...
export function BookingDetailModal({
  ...
  const [savedDraftBillId, setSavedDraftBillId] = useState(
    booking.nhanh_draft_bill_id ?? "",
  );
  ...
  const nhanhQuery = useQuery({
    ...
    queryFn: async () =>
      postJson<{ rows: NhanhRow[] }>("/api/nhanh/draft-products", {
        billId: savedDraftBillId,
      }),
  ...
  const refreshAll = async () => {
    ...
    void queryClient.invalidateQueries({
      queryKey: ["reviewer-amendment", booking.id],
    });
  ...
  const rejectItemMutation = useMutation({
    mutationFn: async ({
      ...
      setActionItemId(itemId);
      await postJson<{ ok: true }>(`/api/reviewer/items/${itemId}/reject`, {
        reason,
      });
    },
    onSuccess: () => {
  ...
  const deleteBookingMutation = useMutation({
    mutationFn: async (bookingId: string) => {
      await postJson<{ ok: true }>(
        `/api/reviewer/bookings/${bookingId}`,
        undefined,
        { method: "DELETE" },
      );
    },
  ...
  const saveDraftBillMutation = useMutation({
    mutationFn: async () => {
      ...
      await postJson<{ ok: true }>(
        `/api/reviewer/bookings/${booking.id}/draft-bill`,
        {
    ...
    onSuccess: async () => {
      ...
      await onBookingRefresh();
      void queryClient.invalidateQueries({
        queryKey: ["nhanh-draft-products"],
      });
    },
  });

  const nhanhRows = nhanhQuery.data?.rows ?? [];
  const nhanhTotal = nhanhRows.reduce(
      ...
      >
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              <p className="text-xs text-[#888888]">Nhà cung cấp</p>
              <p className="font-medium">{booking.supplier_name}</p>
            </div>
            <div>
              <p className="text-xs text-[#888888]">Kho</p>
              <p className="font-medium">{booking.warehouse_name}</p>
            </div>
            <div>
              <p className="text-xs text-[#888888]">Ngày giao</p>
              <p className="font-medium">
                {formatDateDisplay(booking.delivery_date)}
              </p>
            </div>
            <div>
              <p className="text-xs text-[#888888]">Khung giờ</p>
              <p className="font-medium">
          ...
          <div className="rounded border border-[#ecdbe8] bg-white p-3">
            ...
            {saveDraftBillMutation.error && (
              <p className="form-error mt-1">
                {(saveDraftBillMutation.error as Error).message}
              </p>
            )}
          </div>

          {booking.ghi_chu && (
            <div className="p-3 bg-[#F5F5F5] rounded text-sm">
              {booking.ghi_chu}
            </div>
          )}
          ...
          <div className="grid grid-cols-4 gap-2 text-center text-xs">
            <div className="rounded border border-[#ecdbe8] px-2 py-2">
              <p className="font-bold text-[#1a7a3e]">
                {itemCounts.confirmed}/{itemCounts.total}
              </p>
              <p className="text-[#888888]">Đã duyệt</p>
            </div>
            <div className="rounded border border-[#ecdbe8] px-2 py-2">
              <p className="font-bold text-[#CC0000]">
                {itemCounts.rejected}/{itemCounts.total}
              </p>
              <p className="text-[#888888]">Từ chối</p>
            </div>
            <div className="rounded border border-[#ecdbe8] px-2 py-2">
              <p className="font-bold text-[#7A3E00]">
                {itemCounts.returned}/{itemCounts.total}
              </p>
              <p className="text-[#888888]">Trả hàng</p>
            </div>
            <div className="rounded border border-[#ecdbe8] px-2 py-2">
              <p className="font-bold text-[#888888]">
                {itemCounts.pending}/{itemCounts.total}
              </p>
              <p className="text-[#888888]">Chờ xử lý</p>
            </div>
          </div>
          <p className="text-xs text-[#888888]">
            {formatBookingItemSummary(itemCounts)}
          </p>
          ...
          {savedDraftBillId && (
            <div className="rounded border border-[#ecdbe8] bg-[#FFFCF5] p-3 text-sm">
              <div className="flex items-center justify-between gap-3">
                ...
                {nhanhQuery.isFetching && (
                  <span className="text-xs text-[#888888]">Đang tải...</span>
                )}
              </div>
              {nhanhQuery.error ? (
                <p className="mt-2 text-xs text-[#CC0000]">
                  {(nhanhQuery.error as Error).message}
                </p>
              ) : (
                <div className="mt-2 space-y-2">
                  <p
                    className={`text-xs font-medium ${nhanhTotal === bookingTotal ? "text-[#1a7a3e]" : "text-[#CC0000]"}`}
                  >
                  ...
                  return (
                    <tr key={item.id} className="border-t border-[#ecdbe8]">
                      ...
                      <td className="table-cell">
                        {vatPhotos.length > 0 ? (
                          <div className="flex gap-1 flex-wrap">
                            {vatPhotos.map((src: string, i: number) => (
                              <AttachmentThumbnail
                                key={i}
                                src={src}
                                label="VAT"
                      ...
                      <td className="table-cell">
                        {slipPhotos.length > 0 ? (
                          <div className="flex gap-1 flex-wrap">
                            {slipPhotos.map((src: string, i: number) => (
                              <AttachmentThumbnail
                                key={i}
                                src={src}
                                label="Phiếu giao"
                        ...
                        {item.status === "pending" ? (
                          <div className="flex flex-wrap gap-2">
                            ...
                            <Button
                              ...
                              onClick={() => returnItemMutation.mutate(item.id)}
                              className="text-xs py-1 px-2"
                            >
                              Trả hàng
                            </Button>
                          ...
                          <div className="flex flex-col gap-1">
                            {(item.status === "rejected" ||
                              item.status === "returned") &&
                              item.reject_reason && (
                                <p className="max-w-40 text-xs text-[#CC0000]">
                                  {item.reject_reason}
                                </p>
                              )}
                            <Button
              ...
              return (
                ...
                >
                  ...
                  <dl className="mt-3 grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div>
                      <dt className="text-[#888888]">Lần giao / SL</dt>
                      <dd>
                        {item.is_final_round ? "Cuối" : item.delivery_round} /{" "}
                        {itemTotal(item)}
                      </dd>
                  ...
                  {(vatPhotos.length > 0 || slipPhotos.length > 0) && (
                    <div className="mt-3 flex flex-wrap gap-2">
                      {vatPhotos.map((src: string, index: number) => (
                        <AttachmentThumbnail
                          key={`vat-${index}`}
                          src={src}
                          label="VAT"
                      ...
                      {slipPhotos.map((src: string, index: number) => (
                        <AttachmentThumbnail
                          key={`slip-${index}`}
                          src={src}
                          label="Phiếu giao"
            ...
            <div className="flex justify-end">
              <Button
                ...
                onClick={() => {
                  if (
                    window.confirm(
                      `Xoá booking ${booking.booking_code}? Không thể hoàn tác.`,
                    )
                  ) {
        ...
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <Button
              variant="danger-outline"
              loading={
                rejectItemMutation.isPending && actionItemId === rejectItemId
              }
