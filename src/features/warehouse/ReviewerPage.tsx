  ...
  const { data: photos = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      for (const item of data ?? []) {
        const d = item as any
        const vatPhotos = (d.booking_item_photos ?? []).filter((p: any) => p.photo_type === 'vat_invoice')
        if (vatPhotos.length > 0) {
          for (const p of vatPhotos) urls.push({ src: resolvePhotoUrl(p.storage_path), label: 'VAT' })
        } else if (d.vat_invoice_url) {
          urls.push({ src: d.vat_invoice_url, label: 'VAT' })
        }
        for (const p of (d.booking_item_photos ?? [])) {
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {selectedBooking.items.map((item: any) => {
                    ...
                    const vatPhotos = (item.booking_item_photos ?? []).filter((p: any) => p.photo_type === 'vat_invoice')
                    if (vatPhotos.length > 0) {
