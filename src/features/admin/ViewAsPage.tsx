...
const WarehousesPage = lazy(
  () => import("@/features/warehouse/WarehousesPage"),
);
...
function SupplierView() {
  ...
  const { data: bookings = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const result = await getJson<{ bookings: any[] }>(
        "/api/reviewer/admin-view",
      );
      ...
      return (data ?? []).map((b: any) => ({
        ...
        ghi_chu: b.ghi_chu,
        reject_reasons: (b.booking_items ?? [])
  ...
  return (
    <main className="flex-1 lg:w-[80vw] max-w-none mx-auto w-full px-4 py-6">
      <p className="text-xs text-[#888888] mb-4">
        Đang xem giao diện Supplier — hiển thị tất cả booking (admin view)
      </p>
      {bookings.length === 0 ? (
                ...
                {paginatedBookings.map((b) => (
                  <tr key={b.id}>
                    ...
                    <td className="table-cell text-[#888888]">
                      {formatDateTimeDisplay(b.submitted_at)}
                    </td>
                    <td className="table-cell">
                      <StatusBadge status={b.status} />
                    </td>
                    <td className="table-cell max-w-52 text-[#555555]">
                      {b.ghi_chu || <span className="text-[#BBBBBB]">—</span>}
                    </td>
                    <td className="table-cell max-w-52 text-[#CC0000]">
                      {b.reject_reasons || (
  ...
  return (
    <div className="min-h-screen flex flex-col bg-[#fdf8ff]">
      ...
      >
        ...
        {roleView !== "supplier" && pageId === "reviewer" && (
          <ReviewerPage embedded />
        )}
        {roleView !== "supplier" && pageId === "warehouses" && (
          <WarehousesPage embedded />
        )}
        {roleView !== "supplier" && pageId === "suppliers" && (
          <SuppliersPage embedded />
        )}
        {roleView !== "supplier" && pageId === "report" && (
          <ReportPage embedded />
        )}
      </Suspense>
    </div>
