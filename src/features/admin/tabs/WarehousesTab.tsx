  ...
  return (
    <div>
      <div className="flex justify-end mb-3">
        <button onClick={() => { setAdding(true); setNewCode(''); setNewName('') }} className="btn-green">+ Thêm kho</button>
      </div>
                  ...
                  <div className="flex gap-3">
                    {editing?.id === w.id ? (
                      ...
                      <>
                        ...
                        <LinkBtn danger onClick={() => { if (window.confirm(`Xóa kho "${w.name}"? Không thể hoàn tác.`)) deleteMutation.mutate(w.id) }}>Xóa</LinkBtn>
                      </>
                    )}
                  </div>
                </td>
