...
function getPresets() {
  ...
  return [
    ...
    label: p.label,
    start: formatLocalDate(p.start),
    end: formatLocalDate(p.end),
  }));
}
...
function TypedDateInput({
  ...
  return (
    <input
      ...
      className="w-32 rounded border border-[#ecdbe8] px-2 py-1 text-[13px] focus:border-[#80417A] focus:outline-none"
    />
  );
}
    ...
    <div className="w-72">
      <div className="mb-2 flex items-center justify-between gap-2">
        <button
          type="button"
          onClick={onPrev}
          className={`h-7 w-7 rounded text-[15px] ${onPrev ? "hover:bg-[#F5F5F5]" : "invisible"}`}
        >
          ‹
        </button>
        <div className="flex items-center gap-1">
          <select
            value={month}
            onChange={(e) => onMonthYear(year, Number(e.target.value))}
            className="rounded border border-[#ecdbe8] px-1 py-1 text-[15px]"
          >
            {VI_MONTHS.map((m, i) => (
          ...
          <select
            value={year}
            onChange={(e) => onMonthYear(Number(e.target.value), month)}
            className="rounded border border-[#ecdbe8] px-1 py-1 text-[15px]"
          >
            {yearRange.map((y) => (
        ...
        <button
          type="button"
          onClick={onNext}
          className={`h-7 w-7 rounded text-[15px] ${onNext ? "hover:bg-[#F5F5F5]" : "invisible"}`}
        >
          ›
        </button>
      </div>
      <p className="mb-1 text-[13px] font-semibold text-[#888888]">{label}</p>
      ...
      <div className="mt-1 grid grid-cols-7 gap-1">
        {days.map((date) => {
          ...
          let className = "h-8 rounded text-[13px] transition-colors ";
          if (disabled) className += "cursor-not-allowed text-gray-300";
            ...
            className += "border border-[#80417A] font-semibold text-black";
...
export function DateRangePickerPopup({
  ...
  const [leftYM, setLeftYM] = useState<[number, number]>([
  ...
  return (
    <div
      ref={wrapRef}
      className={`relative min-w-[220px] ${className}`}
      style={rootStyle}
    >
      {label && (
        <label className="mb-1 block text-[13px] font-medium text-[#888888]">
          {label}
        </label>
      )}
      <button
        ...
        disabled={isLoading}
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-2 rounded border border-[#ecdbe8] bg-white px-3 py-1.5 text-left text-[15px] transition-colors hover:border-[#80417A] focus:border-[#80417A] focus:outline-none disabled:cursor-not-allowed disabled:opacity-50"
      >
        <span
          className={startDate || endDate ? "text-black" : "text-[#888888]"}
        >
          {triggerText}
        </span>
            ...
            {presets.map((preset) => {
              ...
              return (
                <button
                  ...
                  className={`block w-full rounded px-2 py-1.5 text-left text-[13px] ${active ? "font-semibold text-white" : "text-[#555555] hover:bg-[#F5F5F5]"}`}
            ...
            <div className="mt-4 flex flex-col gap-3 border-t border-[#ecdbe8] pt-3 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={clear}
                  className="rounded border border-[#ecdbe8] px-3 py-1.5 text-[13px] font-medium hover:bg-[#F5F5F5]"
                >
                  Xóa
                </button>
                <button
                ...
                <button
                  type="button"
                  onClick={apply}
                  className="rounded border border-[#80417A] bg-[#80417A] px-3 py-1.5 text-[13px] font-semibold text-white hover:opacity-90"
                >
                  Áp dụng
                </button>
              </div>
