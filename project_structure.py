...
def print_project_summary(root_path=".", output_file="Project_structure.txt"):
    ...
    for root, dirs, files in os.walk(root_path):
        ...
        for file in files:
            if file == '__init__.py' or 'schema' in file.lower():
                continue
        ...
        try:
            ...
            for item in items:
                if item.name in ignore_dirs:
                    continue
                if item.is_file() and (item.name == '__init__.py' or 'schema' in item.name.lower()):
                    continue
    ...
    try:
        ...
        print(f"\n✅ Successfully saved project structure to: {output_file}")
